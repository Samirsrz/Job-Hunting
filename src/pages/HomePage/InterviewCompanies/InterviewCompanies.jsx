    ...
    <section className="mt-20 relative bg-gradient-to-r from-orange-100 to-orange-50 rounded-xl flex flex-col lg:flex-row justify-between lg:h-[410px]">
      <aside className="text-center flex gap-3 flex-col items-center justify-center p-3">
        ...
        <h1 className="font-bold text-2xl">
          {" "}
          {t("Prepareforyour")} <br /> {t("nextinterview")}{" "}
        </h1>
      </aside>
      ...
      <aside className="w-full lg:w-[70%] flex flex-col lg:flex-row gap-3 lg:absolute top-[-19px] right-3 bottom-[-19px] mt-1 mr-7">
        <div className="p-6 bg-white rounded-lg shadow-lg lg:h-[440px] flex flex-col justify-between">
          <h2 className="text-xl font-semibold mb-4 text-center lg:text-balance">
            {t("Interviewquestionsbycompany")}
          </h2>
          ...
          <div className="grid md:grid-cols-1 lg:grid-cols-2 gap-4 ">
            {companies.map((company, index) => (
              <div
                key={index}
                className="px-4 py-2 border rounded-lg flex justify-between items-center shadow-sm hover:shadow-lg transition-shadow duration-200"
              >
